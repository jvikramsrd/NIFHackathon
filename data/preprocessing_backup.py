...
def tile_image_and_mask(
    ...
    for r in range(0, H, stride):
        for c in range(0, W, stride):
            ...
            pad_i = np.zeros((patch_size, patch_size, 3), dtype=np.uint8)
            pad_m = np.zeros((patch_size, patch_size), dtype=np.uint8)
            pad_i[:ph, :pw] = pi
            pad_m[:ph, :pw] = pm
...
def _burn_strip_mask(
    ...
    """
    Burn SHP annotations into a (strip_h, W) uint8 mask for one strip.
    Uses a windowed sub-transform so rasterize maps correctly into the strip.
    """
    ...
    for shp_path in shp_files:
        ...
        class_id, _ = role
        if class_id == 0:
            continue  # infrastructure — skip for segmentation mask
        ...
        # Get strip bounds in CRS coordinates.
        # Note: transform.e is usually negative (pixel height), so:
        # north = f, south = f + H * e
        ...
        # Filter shapes to only those intersecting this strip
...
def _extract_crops_streaming(
    ...
    """
    Extract per-building crops by reading only the bounding box of each
    building from the raster — never loads the full image.
    """
    if built_up_shp is None or not built_up_shp.exists():
        return 0
        ...
        with rasterio.open(str(raster_path)) as src:
            ...
            def _process_crop(item):
                ...
                try:
                    ...
                    win = rasterio.windows.Window(c1, r1, c2 - c1, r2 - r1)  # type: ignore
                    with read_lock:
                        n_b = src.count
                        if n_b >= 3:
            ...
            n_workers = max(1, (os.cpu_count() or 4) - 2)
            with ThreadPoolExecutor(max_workers=n_workers) as executor:
                items = list(gdf.iterrows())
                results = list(
                    tqdm(
                        executor.map(_process_crop, items),
                        total=len(items),
                        desc="      Crops",
                        leave=False,
                    )
...
def _extract_infra_streaming(
    ...
    # Write tile images + label files
    with rasterio.open(str(raster_path)) as src:
        for (tr, tc), pts in tqdm(tile_groups.items(), desc="      Infra", leave=False):
            r2, c2 = min(tr + tile_size, H), min(tc + tile_size, W)
            ph, pw = r2 - tr, c2 - tc
            if ph <= 0 or pw <= 0:
                continue
